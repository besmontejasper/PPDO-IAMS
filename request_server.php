					...
					if ($request_count!=0 && $error==0){
						while ($row=mysqli_fetch_array($results)) {
							...
							if (strtolower($item_name) == strtolower($row['item_name']) && $request_status == "pending") {
								...
								$update2=1;
								$echo = "1";
								mysqli_query($db, "UPDATE item_request SET id='$id', item_name='$item_name', requested_quantity='$new_requested_quantity', item_tag='$item_tag', request_date='$request_date', requested_by='$requested_by', request_status = '$request_status' WHERE id=$id ");
							}
						}
						if ($update2!=1){
							$dt = new DateTime();
							...
							$echo = "2";
							mysqli_query($db, "INSERT INTO item_request (item_name, requested_quantity, item_tag, request_date, requested_by, request_status) VALUES ('$item_name', '$requested_quantity', '$item_tag', '$request_date', '$requested_by', '$request_status')");
						}			
					}	
					else {
						if ($error==0 && $request_status != "cancelled") {
							$dt = new DateTime();
							$d = date_add($dt,date_interval_create_from_date_string("+0 days"));
							$request_date = $d->format('Y-m-d');
							$request_status = "pending";
							$echo = "3";
							mysqli_query($db, "INSERT INTO item_request (id, item_name, requested_quantity, item_tag, request_date, requested_by, request_status) VALUES ('$id', '$item_name', '$requested_quantity', '$item_tag', '$request_date', '$requested_by', '$request_status') ");	
						}
