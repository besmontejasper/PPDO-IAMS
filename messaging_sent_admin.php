...
<body>
	...
	<div class="w3-bar w3-top w3-green w3-large" style="z-index:4">
		...
		<span class="w3-bar-item w3-right">SENT ITEMS</span>
	</div>
	...
	<div class="w3-main" style="margin-left:320px;">
		...
		<?php
		$retrieve_sent = mysqli_query($db, "SELECT * FROM messaging_admin WHERE admin_sent=1"); 
		$sent_count = mysqli_num_rows($retrieve_sent);
		if ($sent_count!=0){
			if ($view==0) { 
				?>
				<table class="w3-table w3-striped w3-bordered w3-border w3-hoverable w3-white">   
					...
					<tr>
						...
						while ($row=mysqli_fetch_array($retrieve_sent)) {
							...
							<td><a href="messaging_sent_admin.php?view=<?php echo $row['id']; ?>"><?php echo $row['message_subject']; ?></a></td>
							<td><a href="messaging_sent_admin.php?view=<?php echo $row['id']; ?>"><?php echo $row['message_recipient']; ?></a></td>
							<td><a href="messaging_sent_admin.php?view=<?php echo $row['id']; ?>"><?php echo $row['date_sent'] ?></a></td>
				...
				$retrieve_sent = mysqli_query($db, "SELECT * FROM messaging_admin WHERE admin_sent=1 AND id='$message_id'");
				while ($row=mysqli_fetch_array($retrieve_sent)) {
					?>
					<br>
					<br>
    ...
    <p class="w3-margin-left w3-border w3-padding-large w3-white w3-margin-right"><?php echo $row['message_body']; ?></p>
		...
		<p>You don't have any sent messages.</p>
		<?php  
	} 
	?>
