...
<body>
	...
	<div class="w3-bar w3-top w3-red w3-large" style="z-index:4"> 
		<span class="w3-bar-item w3-left">PPDO Inventory Archive Management System</span>
	...
	<nav class="w3-sidebar w3-bar-block w3-collapse w3-white w3-animate-left w3-card-2" style="z-index:3;width:320px;" id="mySidebar">
		...
		<a href="messaging_admin.php" class="w3-bar-item w3-button w3-hover-deep-orange"><i class="fa fa-inbox w3-margin-right"></i>Inbox<i class="fa fa-caret-down w3-margin-left"></i></a>
	...
	<div class="w3-main" style="margin-left:320px;">
		...
		<?php
		$retrieve_trash = mysqli_query($db, "SELECT * FROM messaging_admin WHERE admin_trash=1"); 
		...
		if ($trash_count!=0){
			if ($view==0) { 
				...
				<table class="w3-table w3-striped w3-bordered w3-border w3-hoverable w3-white">   
					...
					<tr>
						...
						while ($row=mysqli_fetch_array($retrieve_trash)) {
							...
							<td><a href="messaging_trash_admin.php?view=<?php echo $row['id']; ?>"><?php echo $row['message_subject']; ?></a></td>
							<td><a href="messaging_trash_admin.php?view=<?php echo $row['id']; ?>"><?php echo $row['user_sender']; ?></a></td>
							<td><a href="messaging_trash_admin.php?view=<?php echo $row['id']; ?>"><?php echo $row['date_sent'] ?></a></td>
							<td><a href="message_server_admin.php?restore=<?php echo $row['id']; ?>" class="restore_btn"><i class="fa fa-retweet w3-large w3-hover-text-green"></i></a></td></td>
				...
				$retrieve_trash = mysqli_query($db, "SELECT * FROM messaging_admin WHERE admin_trash=1 AND id='$message_id' ");
				while ($row=mysqli_fetch_array($retrieve_trash)) {
					?>
					<br>
					<br>
					...
					<a href="message_server_admin.php?restore=<?php echo $row['id']; ?>" class="restore_btn w3-bar-item w3-button w3-dark-grey w3-button w3-hover-green w3-margin-left w3-padding-large"  style="transition-duration: 0.3s; margin-top: 3%;"><i class="fa fa-retweet w3-large w3-hover-text-blue"></i>&nbsp;&nbsp;Restore</a>
